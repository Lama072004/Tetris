    ...
    while(1){
        ...
        if (controls_all_buttons_pressed()) {
            ...
            if (controls_all_buttons_pressed()) {
                ...
                display_reset_and_show_hud(score_get_highscore());
                game_running = false;
                // ensure splash will be shown and require 1s wait before starting
                splash_shown = false;
                // Drain any pending button events so splash will display properly
                gpio_num_t ev;
        ...
        if (game_over_flag) {
            ...
            display_reset_and_show_hud(score_get_highscore());
            // show splash on next iteration
            splash_shown = false;
            // Drain any pending button events to avoid immediate splash exit
            gpio_num_t ev;
            while (controls_get_event(&ev)) {}
            vTaskDelay(pdMS_TO_TICKS(100));
            continue;
        ...
        if (!game_running) {
            ...
            // After splash returns, drain any pending events and start quickly
            gpio_num_t ev;
            while (controls_get_event(&ev)) {}
            vTaskDelay(pdMS_TO_TICKS(50)); // small debounce
